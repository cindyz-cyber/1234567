/*
  # Fix Kin Synthesis Function Table References

  1. Updates
    - Update synthesize_kin_profile function to use correct table names
    - Use 'totems' instead of 'seals'
    - Add proper tone attributes (power, action, essence)
*/
...
BEGIN
  ...
  result := jsonb_build_object(
    ...
    'seal', jsonb_build_object(
      'name_cn', totem_data.name_cn,
      'name_en', totem_data.name_en,
      'keywords', totem_data.core_keyword,
      'pineal_gland', totem_data.pineal_gland,
      'heart_chakra', 50,
      'throat_chakra', totem_data.throat_chakra
    ),
    'tone', jsonb_build_object(
      'name_cn', tone_data.name_cn,
      'name_en', tone_data.name_en,
      'power', COALESCE(tone_data.energy_pattern, ''),
      'action', COALESCE(tone_data.life_strategy, ''),
      'essence', COALESCE(tone_data.description, '')
    ),
    'year_2026_guidance', throat_guidance,
    'auto_summary', format(
      '你是 %s（%s）的 %s 能量携带者，处于 %s（%s）的生命底色中。%s 你的调性是 %s，这赋予你 %s 的特质。%s',
      totem_data.name_cn,
      totem_data.name_en,
      tone_data.name_cn,
      ...
      tone_data.name_cn,
      COALESCE(tone_data.description, '独特'),
      throat_guidance
    )
  );
