...
/**
 * ⚡ 从 0 秒开始播放：先等元数据再归零再播放，避免从中间开始
 * 策略：loadedmetadata → 强制 currentTime=0 → 再 play()，确保音乐从 0 针开始
 */
export const createAndPlayAudioFromZero = async (url: string): Promise<HTMLAudioElement | null> => {
  console.group('⚡ [audioManager] 从 0 秒冷启动');
  ...
  try {
    ...
    // 🔥 关键：先设置 src，等元数据加载后再归零再播放，否则浏览器可能从非 0 位置开始
    console.log('📡 设置 audio.src，等待 loadedmetadata...');
    audio.src = url;
    ...
    await new Promise<void>((resolve, reject) => {
      const onLoaded = () => {
        ...
        console.log('✅ 元数据已加载，时长:', audio.duration?.toFixed(1), '秒');
        resolve();
      };
    ...
    audio.pause();
    audio.currentTime = 0;
    console.log('⏮️ 归零: currentTime =', audio.currentTime);
    await new Promise(r => setTimeout(r, 60));
    audio.currentTime = 0;

    console.log('▶️ 开始播放（静音启动）...');
    ...
    setTimeout(() => {
      if (audio.currentTime > 0.5) {
        console.warn('⚠️ 检测到位置异常 (>0.5s)，强制归零');
        audio.currentTime = 0;
      }
      audio.muted = false;
      console.log('🔊 解除静音，volume=0.3，当前 currentTime=', audio.currentTime.toFixed(2));
    }, 100);

    currentGlobalAudio = audio;
    console.log('✅ createAndPlayAudioFromZero 完成，从 0 秒开始');
    console.groupEnd();
    return audio;
  } catch (error) {
